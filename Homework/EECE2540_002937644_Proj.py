...
* Program Design:
*               
*               Designing this program, I began by consulting the TCP_client.py example in the class files to confirm the correct usage of functions (like decode/encode, recv/send, and connect). After confirming, I began by working with some pseudo-code. First and foremost, I connect to the address and port provided in the homework document. I then delve into the actual program flow; the flow was along the lines of [Send Intro Message -> Receive First Expression -> Calculate -> Loop Until Success/Failure -> Save Success Code or Fail]. Overall, the premise was quite simple, and all I had to do to actually compute was tokenize the string, read the operation, and use the integer tokens and operations to receive the actual answer. This was repeated until the success code was received.
*
* Testing:
*
*               Once I began implementing the program as described above, I used the sample server running on localhost to test the program. The first part I tested was the format of the received message. I determined this format to be along the lines of EECE2540 EXPR NUM <SPACE> OPERATION <SPACE> NUM. This allowed me to tokenize the received message into ["EECE2540", "EXPR", "NUM1", "OPERATION", "NUM2"]. I then used an if-elif-else tree to determine what the operation string was equal to using <name_of_token_store>[3]. I then tested the tree by generating a response message, printing it to view, and sending it to the sample server. At this point I encountered an error I had forgotten to take into account: the types I was trying to work with were still strings. The fix here was simple, as I simply type-cast to int(). Once I saw that I was receiving a subsequent message, I ran the statement in a success/failure loop, which ran until I received a success code. I tested this on several ports using the sample server, and received a success each time, so I then used the client with the specified server address. This yielded the success code shown below:
*
* Success Code:
*
